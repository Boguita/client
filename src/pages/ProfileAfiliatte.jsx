...
const handleAffiliateDataRequest = async (dniparams) => {
 ...
 setIsLoading(false);
  ...
  const getDniImg = async () => {
    ...
    window.open(`${BACKEND_URL}/${affiliateData.dni_img_frente}`, '_blank');
 
    
  }
   const getDniImgDorso = async () => {
   
    window.open(`${BACKEND_URL}/${affiliateData.dni_img_dorso}`, '_blank');
  ...
  if (conyugueIndex !== -1) {
    ...
    if (libretaImgArray && Array.isArray(libretaImgArray)) {
      libretaImgArray.forEach((libreta) => {
        window.open(`${BACKEND_URL}/${libreta}`, '_blank');
      });
    } else {
 ...
 const getDniImgHijo = async (familiarId, lado) => {
  ...
  if (imagenDNI) {
    window.open(`${BACKEND_URL}/${imagenDNI}`, '_blank');
  } else {
                          ...
                          <div className='flex flex-col cursor-pointer justify-center w-full xl:w-[16vw] items-center rounded-2xl p-2 mt-5  bg-gray-200'>
                            ...
                            {affiliateData.recibo_sueldo.map((recibo, index) => (
                              ...
                              <div className='flex-col flex justify-center items-center' key={index}>
                                ...
                                <a
                                  href={`${BACKEND_URL}/${recibo}`} // Utiliza la URL de tu API
                                  target="_blank"
                    ...
                    .map((beneficio) => (
                      <div key={beneficio.id}>
                        ...
                        <p className='font-semibold text-md capitalize'>
                          ...
                          <span className='block'>
                            Fecha de Entrega: {beneficio.fecha_entrega ? new Date(beneficio.fecha_entrega).toLocaleDateString("es-AR") : beneficio.fecha_otorgamiento ? new Date(beneficio.fecha_otorgamiento).toLocaleDateString("es-AR") : "Sin fecha"}
                          </span>
                          <span className='block'>
                          ...
                          {beneficio.constancia_img ? (
                            <div>
                              <a className='text-[#006084]' href={`${BACKEND_URL}/${beneficio.constancia_img}`} target="_blank" rel="noopener noreferrer">
                                Ver constancia
                              </a>
                            </div>
