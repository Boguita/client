...
const TableStockEnviado = ({ none = true, initialData, rowsPerPage = 15,  showPagination = true, onUpdateUserData }) => {
...
const handleCheckbox = (afiliado) => {
  ...
  const isSelected = afiliadosSeleccionadosCopy.includes(afiliado.idenviados);

  if (isSelected) {
    // Si está seleccionado, quitarlo de la lista
    const index = afiliadosSeleccionadosCopy.indexOf(afiliado.idenviados);
    afiliadosSeleccionadosCopy.splice(index, 1);
  } else {
    // Si no está seleccionado, añadirlo a la lista
    afiliadosSeleccionadosCopy.push(afiliado.idenviados);
  }
...
const handleCheckBoxAll = () => {
  if (selectedUser.length === data.length) {
    ...
    const allUserIds = data.map((afiliado) => afiliado.idenviados);
    setSelectedUser(allUserIds);
  }
};
const handleExport = async () =>  {
  try {
    const res = await api.post(`/tasks/kit-escolar/envios/excel`, {ids: selectedUser}, {responseType: 'blob'});
    const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `registro_envíos.xlsx`);
          ...
          <table   className="table-auto w-full divide-y-4 divide-[#006084]">
            <thead >
              <tr>
                ...
                <th className="px-2 2xl:px-3 py-3   text-left text-xs leading-4 font-extrabold text-black uppercase tracking-wider">
                  Provincia
                </th>
                ...
                <tr  key={index} className={`text-gray-600 text-sm font-semibold ${index % 2 === 0 ? grayRowClass : whiteRowClass }`}>
                     ...
                     <div  className="flex px-3 py-3 items-center ">
                          ...
                          <input
                            type="checkbox"
                            name="afiliado"
                            id={`afiliado_${row.idenviados}`}
                            value={row.idenviados}
                            className="cursor-pointer custom-checkbox"
                            checked={selectedUser.includes(row.idenviados)}
                          ...
                          />
                          <label
                            htmlFor={`afiliado_${row.idenviados}`}
                            className="check"
                          >
                  ...
                  <td className="px-1 2xl:px-4 text-center py-3 whitespace-no-wrap">{row.utiles_Jardín}</td>
                  <td className="px-1 2xl:px-4 text-center py-3 whitespace-no-wrap">{row.utiles_Primario}</td>
                  <td className="px-1 2xl:px-4 text-center py-3 whitespace-no-wrap">{row.utiles_Secundario}</td>              
