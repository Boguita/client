...
const ListBenefits = ({onUpdateData}) => {
...
const handleCheckbox = (afiliado) => {
  ...
  const isSelected = afiliadosSeleccionadosCopy.includes(afiliado.idStock);

  if (isSelected) {
    // Si está seleccionado, quitarlo de la lista
    const index = afiliadosSeleccionadosCopy.indexOf(afiliado.idStock);
    afiliadosSeleccionadosCopy.splice(index, 1);
  } else {
    // Si no está seleccionado, añadirlo a la lista
    afiliadosSeleccionadosCopy.push(afiliado.idStock);
  }
...
const handleCheckBoxAll = () => {
  if (selectedUser.length === filasFiltradasYOrdenadas.length) {
    // Si todos los usuarios están seleccionados, desmarcarlos todos
    setSelectedUser([]);
  } else {
    // Si no están todos seleccionados, seleccionarlos todos
    const allUserIds = filasFiltradasYOrdenadas.map((afiliado) => afiliado.idStock);
    setSelectedUser(allUserIds);
  }
};
const handleExport = async () =>  {
  try {
    const res = await api.post(`/tasks/stock-escolar/excel`, {ids: selectedUser}, {responseType: 'blob'});
    const url = window.URL.createObjectURL(new Blob([res.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `stock_escolar.xlsx`);
                ...
                <tr  key={index} className={`text-gray-600 text-sm font-semibold ${index % 2 === 0 ? grayRowClass : whiteRowClass }`}>
                   <div className="flex px-3 py-3 items-center ">
                          ...
                          <input
                            type="checkbox"
                            name="afiliado"
                            id={`afiliado_${row.idStock}`}
                            value={row.idStock}
                            className="cursor-pointer custom-checkbox"
                            checked={selectedUser.includes(row.idStock)}
                          ...
                          {/* <Checkbox selected={selectedFamiliares.includes(familiar.id)} /> */}
                               ...
                               <td className="px-2 text-xs text-[#006084] py-3 whitespace-no-wrap">{row.provincia}</td>
                                <td className="px-2  py-3 text-xs whitespace-no-wrap">{row.delegacion}</td>
                              <td className="px-2  py-3 text-xs whitespace-no-wrap">{row.seccional}</td>                              
                                <td className="px-2  py-3 text-xs whitespace-no-wrap">{row.direccion}</td>
                                                               <td className={`px-2 2xl:px-6 capitalize py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.mochila < 10 ? 'bg-red-400 text-red-500' : row.mochila >= 10 && row.mochila <= 50 ? 'bg-yellow-400 text-yellow-500' : 'bg-green-400 text-green-500'}`}>{row.mochila}</span></td>
                                <td className={`px-2 text-xs capitalize py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.utiles_Jardín < 10 ? 'bg-red-400 text-red-500' : row.utiles_Jardín >= 10 && row.utiles_Jardín <= 50 ? 'bg-yellow-400 text-yellow-500' : 'bg-green-400 text-green-500'}`}>{row.utiles_Jardín}</span></td>

                                <td className={`px-2 text-xs capitalize py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.utiles_Primario < 10 ? 'bg-red-400 text-red-500' : row.utiles_Primario >= 10 && row.utiles_Primario <= 50 ? 'bg-yellow-400 text-yellow-500' : 'bg-green-400 text-green-500'}`}>{row.utiles_Primario}</span></td>
                                 <td className={`px-2 text-xs capitalize py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.utiles_Secundario < 10 ? 'bg-red-400 text-red-500' : row.utiles_Secundario >= 10 && row.utiles_Secundario <= 50 ? 'bg-yellow-400 text-yellow-500' : 'bg-green-400 text-green-500'}`}>{row.utiles_Secundario}</span></td>
                  <td className={`px-2 text-xs capitalize py-3 whitespace-no-wrap`}>
                      <span className={`bg-opacity-30 text-xs rounded-lg px-2 p-1 ${row.talle6 < 10 ? 'bg-red-400 text-red-500' : row.talle6 >= 10 && row.talle6 <= 50 ? 'bg-yellow-400 text-yellow-500' : 'bg-green-400 text-green-500'}`}>
                        {row.talle6}
                      </span>
                    </td>
