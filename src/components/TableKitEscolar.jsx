...
const TableKitEscolar = ({ initialData, rowsPerPage = 15,  showPagination = true, onUpdateUserData }) => {
...
const handleCheckbox = (afiliado) => {
  const afiliadosSeleccionadosCopy = [...selectedUser];
  ...
  setSelectedUser(afiliadosSeleccionadosCopy);
};


const handleCheckBoxAll = () => {
  if (selectedUser.length === data.length) {
...
const handleGenerateExcel = async () => {
  try {
    ...
    const dataToConvert = [...data]
      .filter((afiliado) => selectedUser.includes(afiliado.id));

    console.log("datos a pasar", dataToConvert);

    // Enviar beneficios al backend para generar el archivo Excel
    const response = await api.post(
      '/tasks/kit-escolar/generate-excel',
      { beneficios: dataToConvert },
      {
        headers: {
    ...
    // Verificar los datos del Blob
    const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
    console.log('Datos del Blob:', response.data);

    // Configurar el nombre del archivo
    const fileName = 'kit_escolar.xlsx';
    ...
    // Configurar el atributo de descarga y agregar el enlace al cuerpo del documento
  ...
  return (
    <div ref={animationParent} className="w-full h-full bg-gray-200">
      { !data ? <Loader /> :
      <div  className="flex flex-col">
        <div ref={animationParent} className="mt-4  bg-white min-h-[25rem] p-8 rounded-xl">
          ...
          <button onClick={handleGenerateExcel} className='bg-[#006084] mb-2 text-white font-semibold rounded-lg p-2 hover:bg-opacity-75'>Exportar seleccionados</button>
          </div>
          ...
          <table   className="w-full table-auto divide-y-4 divide-[#006084]">
            <thead >
              ...
              <th className=" ">
                ...
                <th className=" px-1 py-3   text-left text-xs leading-4 font-extrabold text-black uppercase tracking-wider">
                  Provincia
                </th>
                <th className="px-1  py-3  text-left text-xs leading-4 font-extrabold text-black uppercase tracking-wider">
                  Delegación
                ...
                <th className=" px-1 py-3  text-left text-xs leading-4 font-extrabold text-black uppercase tracking-wider">
                 ...
                 <th className="px-2 2xl:px-2 py-3  text-left text-xs leading-4 font-extrabold text-black uppercase tracking-wider">
                  Acciones
                </th>              
            ...
            <tbody ref={animationParent}>
              {data?.slice(startIndex, endIndex).map((row, index) => (
                <tr  key={index} className={`text-gray-600 text-sm font-semibold ${index % 2 === 0 ? grayRowClass : whiteRowClass }`}>
                  ...
                  <td className="px-1 py-3 text-xs capitalize text-[#006084] whitespace-no-wrap">{row.provincia}</td>
                  <td className="px-1 text-xs capitalize py-3 whitespace-no-wrap">{row.delegacion}</td>
                  <td className="px-1 text-xs capitalize py-3 whitespace-no-wrap">{row.seccional}</td>
                  <td className="px-1 text-xs capitalize py-3 whitespace-no-wrap">{row.direccion}</td>
                  <td className="px-1 capitalize py-3 whitespace-no-wrap">{row.afiliado_name}</td>
                   <td className="px-1 capitalize py-3 whitespace-no-wrap">{row.afiliado_dni}</td>
                  <td className="px-1 capitalize py-3 whitespace-no-wrap">{row.familiar_name}</td>
                  <td className="px-1 py-3 whitespace-no-wrap">{row.familiar_dni}</td>                  
                  <td className="px-1 py-3 whitespace-no-wrap">{row.año_escolar}</td> 
                  <td className={`px-1 py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.utiles === 0 ? 'bg-red-400 text-red-500' : "bg-green-400 text-green-500"}`}>{row.utiles === 0 ? "No" : "Si"}</span></td>   
                  <td className="px-1 py-3 whitespace-no-wrap"><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.guardapolvo_confirm === 0 ? 'bg-red-400 text-red-500' : "bg-green-400 text-green-500"}`}>{row.guardapolvo_confirm === 0 ? "No" : "Si"}</span> T: {row.guardapolvo}</td> 
                  <td className={`px-1 py-3 whitespace-no-wrap`}><span className={`bg-opacity-30 rounded-lg px-2 p-1 ${row.mochila === 0 ? 'bg-red-400 text-red-500' : "bg-green-400 text-green-500"}`}>{row.mochila === 0 ? "No" : "Si"}</span></td>   
  ...
  {!row.utiles && !row.mochila && !row.guardapolvo_confirm ? "" : new Date(row.fecha_otorgamiento).toLocaleString('es-AR', { timeZone: 'America/Argentina/Buenos_Aires' })}
                  ...
                  <td className="flex items-center px-6 py-2 whitespace-no-wrap">
        ...
        rel="noopener noreferrer">Ver ficha completa</a></li>
                            ...
                            <li className='hover:border-[#006084] hover:border-b-2  cursor-pointer'><a href={`${BACKEND_URL}/${row.constancia_img ? row.constancia_img : ""}`}  target="_blank"
